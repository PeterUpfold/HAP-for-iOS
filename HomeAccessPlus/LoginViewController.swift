...
class LoginViewController: UIViewController, UITextFieldDelegate, UIPickerViewDelegate, UIPickerViewDataSource, MFMailComposeViewControllerDelegate {
    ...
    override func viewWillAppear(_ animated: Bool) {
        ...
        if args.contains("UI_TESTING_RESET_SETTINGS") {
            ...
            NotificationCenter.default.addObserver(self, selector: #selector(showHAPServerControls), name: UIApplication.willEnterForegroundNotification, object: nil)
    ...
    @IBAction func attemptLogin(_ sender: AnyObject) {
        ...
        if (textboxesValid() == false) {
            let textboxesValidAlertController = UIAlertController(title: "Incorrect Information", message: "Please check that you have entered all correct information, then try again", preferredStyle: UIAlertController.Style.alert)
            textboxesValidAlertController.addAction(UIAlertAction(title: "OK", style: UIAlertAction.Style.default, handler: nil))
            ...
            if(api.checkConnection()) {
                ...
                let apiCheckConnectionController = UIAlertController(title: "Unable to access the Internet", message: "Please check that you have a signal, then try again", preferredStyle: UIAlertController.Style.alert)
                apiCheckConnectionController.addAction(UIAlertAction(title: "OK", style: UIAlertAction.Style.default, handler: nil))
    ...
    /// - version: 2
    /// - date: 2017-06-04
    @objc func showHAPServerControls() {
    ...
    @objc func checkAPI(_ hapServer: String, attempt: Int) -> Void {
        ...
        self.api.loginUser(self.hapServerAddress, username: self.tblUsername.text!, password: self.tbxPassword.text!, callback: { (result: Bool) -> Void in
            ...
            if (result == true) {
                ...
                if let deviceType = settings!.string(forKey: settingsDeviceType) {
                    ...
                    let loginUserDeviceType = UIAlertController(title: "Please Select Device Type", message: "Personal - A device you have bought\nShared - School owned class set\nSingle - School owned 1:1 scheme", preferredStyle: UIAlertController.Style.alert)
                    loginUserDeviceType.addAction(UIAlertAction(title: "Personal", style: UIAlertAction.Style.default, handler: {(alertAction) -> Void in
                        self.setDeviceType("personal") }))
                    loginUserDeviceType.addAction(UIAlertAction(title: "Shared", style: UIAlertAction.Style.default, handler: {(alertAction) -> Void in
                        self.setDeviceType("shared") }))
                    loginUserDeviceType.addAction(UIAlertAction(title: "Single", style: UIAlertAction.Style.default, handler: {(alertAction) -> Void in
                ...
                let loginUserFailController = UIAlertController(title: "Invalid Username or Password", message: "The username and password combination is not valid. Please check and try again", preferredStyle: UIAlertController.Style.alert)
    ...
    @objc func setDeviceType(_ deviceType: String) {
    ...
    @objc func registerForKeyboardNotifications() {
        ...
        NotificationCenter.default.addObserver(self, selector: #selector(LoginViewController.keyboardWasShown(_:)), name: UIResponder.keyboardWillShowNotification, object: nil)
        NotificationCenter.default.addObserver(self, selector: #selector(LoginViewController.keyboardWillBeHidden(_:)), name: UIResponder.keyboardWillHideNotification, object: nil)
    }
    ...
    */
    ...
    @objc func loadMultisiteList() -> Bool {
    ...
    func pickerView(_ pickerView: UIPickerView, viewForRow row: Int, forComponent component: Int, reusing view: UIView?) -> UIView {
        ...
        var myTitle = NSAttributedString(string: titleData, attributes: convertToOptionalNSAttributedStringKeyDictionary([convertFromNSAttributedStringKey(NSAttributedString.Key.font):UIFont.systemFont(ofSize: 17),convertFromNSAttributedStringKey(NSAttributedString.Key.foregroundColor):UIColor.black]))
        if (row == hapServerPickerData.count - 1) {
            myTitle = NSAttributedString(string: titleData, attributes: convertToOptionalNSAttributedStringKeyDictionary([convertFromNSAttributedStringKey(NSAttributedString.Key.font):UIFont.systemFont(ofSize: 17, weight: UIFont.Weight.semibold),convertFromNSAttributedStringKey(NSAttributedString.Key.foregroundColor):UIColor.black]))
        }
    ...
    @objc func pickUpValue(textField: UITextField) {
    ...
    @objc func selectClick() {
        ...
        if (hapServerPickerCurrentRow == (hapServerPickerData.count - 1)) {
            ...
            let action = UIAlertAction(title: "Continue", style: UIAlertAction.Style.default, handler: {(paramAction:UIAlertAction!) in
            ...
            newServerAlert!.addAction(UIAlertAction(title: "Cancel", style: UIAlertAction.Style.cancel, handler: nil))
...
extension UITextField
{
    ...
    @objc public func hideAssistantBar()
    {
