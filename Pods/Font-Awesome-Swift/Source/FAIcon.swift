...
public extension UIBarButtonItem {
    ...
    func setFAIcon(icon: FAType, iconSize: CGFloat) {
        ...
        setTitleTextAttributes(convertToOptionalNSAttributedStringKeyDictionary([convertFromNSAttributedStringKey(NSAttributedString.Key.font): font!]), for: .normal)
        title = icon.text
    }
...
public extension UIButton {
    ...
    func setFAText(prefixText: String, icon: FAType?, postfixText: String, size: CGFloat?, forState state: UIControl.State, iconSize: CGFloat? = nil) {
        ...
        attributedText.enumerateAttribute(NSAttributedString.Key(rawValue: convertFromNSAttributedStringKey(NSAttributedString.Key.foregroundColor)), in:NSMakeRange(0,attributedText.length), options:.longestEffectiveRangeNotRequired) {
            value, range, stop in
            if value != nil {
        ...
        let textAttributes = [convertFromNSAttributedStringKey(NSAttributedString.Key.font): textFont!, convertFromNSAttributedStringKey(NSAttributedString.Key.foregroundColor): textColor] as [String : Any]
        let prefixTextAttribured = NSMutableAttributedString(string: prefixText, attributes: convertToOptionalNSAttributedStringKeyDictionary(textAttributes))
        
        if let iconText = icon?.text {
            let iconFont = UIFont(name: FAStruct.FontName, size: iconSize ?? size ?? titleLabel.font.pointSize)!
            let iconAttributes = [convertFromNSAttributedStringKey(NSAttributedString.Key.font): iconFont, convertFromNSAttributedStringKey(NSAttributedString.Key.foregroundColor): textColor] as [String : Any]

            let iconString = NSAttributedString(string: iconText, attributes: convertToOptionalNSAttributedStringKeyDictionary(iconAttributes))
            prefixTextAttribured.append(iconString)
        }
        let postfixTextAttributed = NSAttributedString(string: postfixText, attributes: convertToOptionalNSAttributedStringKeyDictionary(textAttributes))
    ...
    @objc func setFATitleColor(color: UIColor, forState state: UIControl.State = .normal) {
        FontLoader.loadFontIfNeeded()
...
public extension UILabel {
    ...
    @objc func setFAColor(color: UIColor) {
        FontLoader.loadFontIfNeeded()
    ...
    func setFAText(prefixText: String, icon: FAType?, postfixText: String, size: CGFloat?, iconSize: CGFloat? = nil) {
        ...
        let textAttribute = [convertFromNSAttributedStringKey(NSAttributedString.Key.font) : textFont!]
        let prefixTextAttribured = NSMutableAttributedString(string: prefixText, attributes: convertToOptionalNSAttributedStringKeyDictionary(textAttribute))
        
        if let iconText = icon?.text {
            let iconFont = UIFont(name: FAStruct.FontName, size: iconSize ?? size ?? font.pointSize)!
            let iconAttribute = [convertFromNSAttributedStringKey(NSAttributedString.Key.font) : iconFont]
            
            let iconString = NSAttributedString(string: iconText, attributes: convertToOptionalNSAttributedStringKeyDictionary(iconAttribute))
            prefixTextAttribured.append(iconString)
        }
        let postfixTextAttributed = NSAttributedString(string: postfixText, attributes: convertToOptionalNSAttributedStringKeyDictionary(textAttribute))
        prefixTextAttribured.append(postfixTextAttributed)
...
public extension UITabBarItem {
    ...
    public func setFAIcon(icon: FAType, size: CGSize? = nil, orientation: UIImage.Orientation = UIImage.Orientation.down, textColor: UIColor = UIColor.black, backgroundColor: UIColor = UIColor.clear, selectedTextColor: UIColor = UIColor.black, selectedBackgroundColor: UIColor = UIColor.clear) {
        ...
        setTitleTextAttributes(convertToOptionalNSAttributedStringKeyDictionary([convertFromNSAttributedStringKey(NSAttributedString.Key.foregroundColor): textColor]), for: .normal)
        setTitleTextAttributes(convertToOptionalNSAttributedStringKeyDictionary([convertFromNSAttributedStringKey(NSAttributedString.Key.foregroundColor): selectedTextColor]), for: .selected)
    }
}
...
public extension UISegmentedControl {
    ...
    public func setFAIcon(icon: FAType, forSegmentAtIndex segment: Int) {
        ...
        let font = UIFont(name: FAStruct.FontName, size: 23)
        assert(font != nil, FAStruct.ErrorAnnounce)
        setTitleTextAttributes([NSAttributedString.Key(rawValue: convertFromNSAttributedStringKey(NSAttributedString.Key.font)): font!], for: .normal)
...
public extension UIImage {
    ...
    public convenience init(icon: FAType, size: CGSize, orientation: UIImage.Orientation = UIImage.Orientation.down, textColor: UIColor = UIColor.black, backgroundColor: UIColor = UIColor.clear) {
        ...
        assert(font != nil, FAStruct.ErrorAnnounce)
        let attributes = [convertFromNSAttributedStringKey(NSAttributedString.Key.font): font!, convertFromNSAttributedStringKey(NSAttributedString.Key.foregroundColor): textColor, convertFromNSAttributedStringKey(NSAttributedString.Key.backgroundColor): backgroundColor, convertFromNSAttributedStringKey(NSAttributedString.Key.paragraphStyle): paragraph]
        
        let attributedString = NSAttributedString(string: icon.text!, attributes: convertToOptionalNSAttributedStringKeyDictionary(attributes))
...
public extension UIViewController {
    var FATitle: FAType? {
        set {
            ...
            let font = UIFont(name: FAStruct.FontName, size: 23)
            assert(font != nil,FAStruct.ErrorAnnounce)
            navigationController?.navigationBar.titleTextAttributes = convertToOptionalNSAttributedStringKeyDictionary([NSAttributedString.Key.font.rawValue: font!])
            title = newValue?.text
        }
        get {
            guard let title = title, let index = FAIcons.index(of: title) else { return nil }
